...
app.page = function(path, fns) {
  ...
  // TODO: Driver for static, parameter-less paths
  /*
...
app.handle = function(page, callback) {
  ...
  // TODO: Accumulate list of pages for building sitemaps, etc
...
app.generate = function(cb) {
  ...
  this._bind(queue, function(err) {
    ...
    for (i = 0, len = drivers.length; i < len; ++i) {
      ...
      drivers[i].driver.on('request', bindSite(drivers[i].site, pool2, drivers[i].mountpath));
      drivers[i].driver.go();
    }
  });
}
