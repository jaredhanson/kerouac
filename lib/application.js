...
app.defaultConfiguration = function defaultConfiguration() {
  ...
  this.set('output', process.cwd() + '/www'); // TODO: resolve the path
...
app.handle = function handle(page, callback) {
  ...
  // final handler
  var done = callback || function(err, page) {
...
app.engine = function engine(ext, fn, options) {
  ...
  if (typeof fn == 'function') {
    ...
    mod.renderFile = fn.renderFile;
    mod.render = fn.render;
    mod.options = options;
  }
  if (typeof mod.renderFile != 'function') {
    throw new Error('renderFile function required');
  }
...
app.render = function(name, options, callback, file) {
  ...
  if (file) {
    ...
    if (engine.options) merge(opts, engine.options);
    merge(opts, options);
    
    if ('function' != typeof engine.render) throw new Error('string rendering not supported by "' + ext + '" engine');
    engine.render(name, opts, done);
  }
}

app.convert = function(str, options, callback) {
  var opts = {};
  var done = callback;
  
  // TODO: Respect default layout engine setting?  or have a new markup setting?
  var ext = options.engine || 'md';
  if ('.' != ext[0]) ext = '.' + ext;
...
app.generate = function(clients, cb) {
  ...
  if (!clients) {
    clients = [ crawlers.contentCrawler() ]
  }
  
  
  var self = this;
  var pool = new fnpool.Pool({ size: 1 }, function generate(page, done) {
  ...
  */
  ...
  var server = ssgp.createServer(function(page) {
    pool.task(page);
  });
  ...
  server.on('disconnection', function() {
    ...
    var n = this.getConnections();
    if (n !== 0) { return; }
    
    console.log('DOING DEFERRED!');
    
    var client, i, len;
    for (i = 0, len = deferred.length; i < len; ++i) {
      client = deferred[i];
      server.accept(client[1], client[0]);
    }
    deferred = [];
  });
  ...
  for (i = 0, len = clients.length; i < len; ++i) {
    ...
    if (Array.isArray(client)) {
      if (typeof client[0] == 'object') {
        path = '/';
        c = client[0];
        defer = client[1] == undefined ? false : !client[1];
      }
      else if (typeof client[0] == 'string') {
        path = client[0];
        c = client[1];
        defer = client[2] == undefined ? false : !client[2];
      }
      
      if (!defer) {
        server.accept(c, path);
      } else {
        deferred.push([ path, c ]);
      }
    } else {
