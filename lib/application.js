...
app.set = function(setting, val) {
  ...
  if (1 == arguments.length) {
    return this.settings[setting] || (this.parent ? this.parent.get(setting) : undefined);
  } else {
...
app.generate = function(cb) {
  ...
  this._bind(queue, function(err) {
    ...
    function dispatch(req, done) {
      ...
      //page.site = self;
      //page.pages = pages;
      page.baseURL = self.get('base url');
      self.handle(page);
    }
