...
/**
 * Register the given front matter parsing `fn` for `delim`.
 *
 ...
 * site.fm(';;;', JSON.parse);
 */
app.fm = function(delim, fn) {
  if (typeof fn !== 'function') {
    throw new Error('callback function required');
  }
  
  // store front matter
  this.frontmatters[delim] = fn;
  return this;
}
...
/**
 * Convert lightweight markup of type `type` to type `otype`.
 *
 * @public
 * @param {string} text
 * @param {string} type
 * @param {string} [otype='html']
 * @param {string} [options]
 * @returns {string}
 */
app.convert = function(text, type, otype, options) {
  otype = otype || 'html';
  options = options || {};
  ...
  if (!markup) {
    ...
    throw new Error('Failed to convert markup from "' + extension + '" to "' + oextension + '"');
  }
  
  return markup(text, opts);
}

/**
 * Parse front matter delimited `delim`.
 *
 * @public
 * @param {string} text
 * @param {string} [delim='---']
 * @returns {object}
 */
app.parse = function(text, delim) {
  delim = delim || '---'
  
  if (!text) { return; } // no front matter
  
  var frontmatter = this.frontmatters[delim];
  if (!frontmatter) {
    throw new Error('Failed to parse front matter with delimiter "' + delim + '".');
  }

  var data = frontmatter(text);
  if (typeof data !== 'object') {
    throw new Error('Front matter must consist of fields and values')
  }
  return data;
}
