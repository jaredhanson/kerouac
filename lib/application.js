...
app.use = function use(fn) {
  ...
  if (typeof args[0].createMapper == 'function') {
    ...
    this._mappers.push({ mapper: args[0].createMapper.apply(undefined, args.slice(1)), path: path });
  } else {
...
app.generate = function(clients, cb) {
  ...
  server.accept(this._mapper);
  
  for (i = 0, len = this._mappers.length; i < len; ++i) {
    client = this._mappers[i];
    console.log('AUTO MAPPING: ' + client.path);
    server.accept(client.mapper, client.path);
  }
