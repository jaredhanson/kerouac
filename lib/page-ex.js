...
page.convert = function(text, type, otype, options, callback) {
  ...
  // support callback function as third or fourth
  if (typeof otype === 'function') {
    done = otype;
    opts = {};
    to = 'html';
  } else if (typeof options === 'function' && typeof otype !== 'object') {
    done = options;
    opts = {};
  }
  ...
  var out = app.convert(text, type, to, opts);
  done(null, out);
};
...
page.compile = function(text, type, layout, options, callback) {
  ...
  // TODO: make this an optional sync function like render??
  var out = app.convert(text, type);
  done(null, out);
};

// TODO: add a page.convert here, which converts and writes, bypassing layout
