...
Kerouac.prototype.bind = function(fn) {
  if ('function' == typeof fn.generate) {
    var server = fn;
    fn = function(queue, cb) {
      server._bind(queue, cb);
    };
  }
...
Kerouac.prototype._bind = function(queue, cb) {
  cb = cb || function() {};
  ...
  (function iter(i, err) {
    ...
    block.call(self, new Queue(self.path, queue));
    iter(i + 1);
  })(0);
}
